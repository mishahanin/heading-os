...
| `/canopus [note \| check \| where \| probe]` | NEVER auto-trigger. Explicit `/canopus [note \| check \| where \| probe]` only. Bare `/canopus` prints the current position and the full agenda. | N/A | No |
