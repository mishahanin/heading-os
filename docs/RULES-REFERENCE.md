...
| `classification.md` | Path-scoped | Record classification (engine / private / corporate) driven by `config/routing-map.yaml`. The fail-closed direction that keeps real data out of the engine. Path-scoped on 2026-09-04 to the four ask-directories (`context/`, `reference/`, `knowledge/`, `datastore/`) and the routing map, because the obligation fires on a file path, which the harness can match. Two limits it names rather than hides: a file created through `Bash` matches no `file_path` argument, and a path outside the globs takes the `engine` default. Neither is a seam hole, because `leak-guard.py`, `engine_guard.py` and the push-time content scan refuse private content at the push regardless. |
...
| `skill-router.md` | Always-on | Matches a natural-language message to the right skill: the precedence order, the compound-trigger handoff, and a generated registry row per skill. Two layers since the 2026-09-04 context diet. The resident core index carries only what is needed to reach a CANDIDATE, which means trigger phrases for the auto-routable skills and the marker `manual` for the 23 that no message can reach; exclusions, compound patterns and the explicit-only skills' flags and argument grammar live in `reference/skill-router/<category>.md`, read before selecting. The auto-routable trigger cells were deliberately NOT cut: the harness evicts skill descriptions least-invoked-first under a context-window-fraction budget, so for a low-traffic skill the router row is the only matching surface guaranteed to be resident. Generated by `scripts/generate-skill-router.py`; never hand-edited. |
...
| `console-first.md` | Always-on | Every capability must be operable from the terminal and chat. The web dashboard is a convenience layer, never a dependency. Stays resident because choosing a browser-only design happens before any file exists, so no path can fire in time; the build checklist behind it moved out on 2026-09-04. |
| `console-first-build.md` | Path-scoped | What the console-first check actually consists of: the four build requirements (non-web path first, backing store as source of truth, never a web-only writer, degrade non-zero rather than hang) and the three validation questions. Scoped to `scripts/**`, `.claude/skills/**` and `.claude/agents/**`, so it loads on the first write to any capability file. |
| `memory-discipline.md` | Always-on | Before any consequential action, open the authoritative record, not the pointer that surfaced it (an index hook, recalled snippet, or summary). The Fidelity half stays resident because it fires on a judgement about consequence that nothing mechanical can signal; the Economy half (hooks carry topic + pointer, never volatile state, enforced advisory by `scripts/memory-hygiene.py`) keeps a resident summary with the detail in `reference/memory-discipline-detail.md`, which `/memory-hygiene` and `/dream` open before touching the index. |
...
| Rule | Scope | What it enforces |
|------|-------|------------------|
| `corporate-docs.md` | Always-on | Five external doctypes (letter, proposal, partnership doc, official doc, one-pager) auto-route to their locked branded templates with an announced skill selection. Stays resident because it fires on a MESSAGE, which no path can match. |
| `corporate-docs-authoring.md` | Path-scoped | Everything that fires only after a doctype has been chosen: brand enforcement, the rendering pipeline, output naming, classification, and change control on the locked templates. Loads when a skill opens `reference/corporate-style-guide.md`, which the always-on guardrail requires it to do, or when the renderer, the templates or `outputs/documents/` are touched. |
...
| `visual-design-discipline.md` | Path-scoped | Design every visual so it reads as intentional and human-made, not a templated default. |
...
| Rule | Scope | What it enforces |
|------|-------|------------------|
| `development-standards.md` | Path-scoped | Quality gates for every artifact: the Evidence Standard (a change carries evidence that it can fail, in twelve obligations, each naming the gate that holds it or saying none does), research first, restraint (simplicity and surgical changes), a debugging discipline, and the skill / script / reference standards. |
...
| `documentation-style.md` | Path-scoped | A subset of ASD-STE100 Simplified Technical English for the procedural pages and skill instruction bodies: imperative steps, active voice, a word limit per sentence, one action per step, and a warning that arrives before the step it guards. Audited by `scripts/ste-check.py`, which leaves its heuristic warnings advisory. Both halves gate on errors: `--all` fails a commit on the fourteen procedural pages, `--skills` on the skill bodies. Every Markdown page under `docs/` is named in one list or the other, and a test fails until a new page is classified. The hand-authored HTML pages, which have no Markdown source, are classified on a separate axis and stay outside the ASD-STE100 gate: the checker parses Markdown, so it would read HTML as tag soup. Explanatory pages and all outbound prose stay out of scope, where `humanization.md` governs instead. |
| `persistence.md` | Always-on | State lives in files, in any format that fits, and the workspace's database is SQLite. Postgres, Oracle, MySQL, Microsoft SQL Server, Redis, Qdrant, Neo4j and every other SERVER database is ruled out rather than weighed, because a store that outlives the process that wrote to it buys a daemon, a port and a backup story for a durability guarantee WSL2 does not document anyway. An in-process store adds none of that and is not ruled out. Stays resident because it fires when you propose something that persists, which happens in prose before a file is touched. The two carve-outs written out, why each was a correction, and the validation step: `reference/persistence-detail.md`. Rationale in `docs/ARCHITECTURE.md` § 4. |
...
| `scope-claims.md` | Path-scoped | A tool reports the coverage its method establishes and no more: resolve the claim, name what was left out, and widen rather than go silent when the evidence is missing. Enforced by `tests/test_scope_claims.py`, which makes every session-membership or live-execution claim declare what resolves it. The registry is keyed per CLAIM (path plus a hash of the normalised sentence), not per file, so a second claim in an already-declared file cannot inherit the first one's answer. |
