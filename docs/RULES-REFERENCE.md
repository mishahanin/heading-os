...
| `persistence.md` | Always-on | State lives in Markdown files or SQLite, and in nothing else. Postgres, Redis, Qdrant, Neo4j and every other server-process database are ruled out rather than weighed, because a store that outlives the process that wrote to it buys a daemon, a port and a backup story for a durability guarantee WSL2 does not document anyway. Rationale in `docs/ARCHITECTURE.md` § 4. |
