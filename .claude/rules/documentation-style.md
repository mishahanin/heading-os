...
**Every `.md` page under `docs/` sits in one of those two lists, and a test holds it there.** Until 2026-08-22 three did not: `EXTENDING.md`, `TELEGRAM-AND-ALERTS.md` and `RULES-REFERENCE.md` were absent from the frontmatter and absent from the exclusion sentence, so no one had ever decided about them. Two turned out to be pages a reader executes and joined the gate at a cost of 28 sentences; one is a catalogue and joined the exclusion list. The failure is worth naming because of its shape: a page fell through by being in NEITHER list, which no gate on the gated set can ever see. `test_every_docs_page_is_classified` reads this paragraph and the frontmatter together, so a new `.md` page under `docs/` fails the suite until someone decides which list it belongs to.

**The `docs/` HTML pages are classified on a second axis, and none of them is inside the ASD-STE100 gate.** Most are GENERATED from a sibling `.md` by `scripts/regenerate-docs-html.py`, so a generated page's classification is already its source's and nothing further is decided about it here. The rest are HAND-AUTHORED, with no `.md` behind them, and the line below is the complete list.

`daemons.html`, `data-structure.html`, `index.html`, `memory-odin.html`, `prerequisites.html`, `skills-communication.html`, `skills-content-design.html`, `skills-crm.html`, `skills-intel.html`, `skills-mcp-plugins.html`, `skills-operations-daily.html`, `skills-operations-infra.html`, `skills-operations-quality.html`, `skills-strategy.html`

That line is the authority and no numeral here counts it, because a prose count is the part that goes stale. It is a line of nothing but names for a reason: the parser that reads it takes the whole line, so a page named in passing in the surrounding prose would otherwise enter the list silently. That is not hypothetical. The first version of this paragraph carried the names inline, a mutation removed `prerequisites.html` from them, and the test stayed green because the reasoning below mentions the same page twice. Those pages sit outside the gate deliberately. `scripts/ste-check.py` parses Markdown: it counts an inline code span as one word, it recognises numbered steps, and it honours `<!-- ste-skip -->`. Fed HTML it measures tag soup, so gating them means writing an HTML-to-prose extractor first, against this checker's own history of three rounds of splitter defects in which 83 sentences were rewritten to satisfy a broken measurement. That is a large cost to repeat on a second markup language for a low yield: every page on the list except `prerequisites.html` is a catalogue, which is the register the exclusion sentence above already protects. `prerequisites.html` is not a catalogue, it is a from-scratch install page, so it carries a content gate rather than a prose gate: `test_prerequisites_pinned_packages_are_actually_declared` in `tests/test_docs_pages_that_pointed_at_nothing.py` holds its pinned-package claims to what `pyproject.toml` actually declares. Until 2026-08-31 none of this was written down and none of it was tested, because `test_every_docs_page_is_classified` globbed `*.md` only while the sentence above claimed every page, and `excluded_page_names()` could not have named an HTML page even if someone had wanted to: its regex accepts `.md` and nothing else. `test_every_docs_html_page_is_classified` now reads this list against the disk in both directions, so a new hand-authored page fails the suite until it is named here, and a name that is no longer hand-authored fails just as loudly.
