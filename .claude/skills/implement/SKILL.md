...
- **Wave bracketing.** `wave_start` precedes the wave's first `step_start`; `wave_end` follows all member `step_end`s; `wave_end.successes` equals the count of bracketed `step_end` with status `ok`/`deviation` (not the declared membership). Once a run emits any wave event, EVERY step but step 0 belongs to a bracket, and every `wave_start` carries `--step-count` and `--parallel`/`--no-parallel`. **`--verify` checks all three since v1.8** (an orphan `wave_end` no longer buys its `successes` claim a free pass).
...
3. **Handle issues gracefully:**
   ...
   - **Emit a `deviation` event** when you adapt a step. Emit one as well when the work diverges from explicit plan text at all. Even a trivial functionally-identical substitution, such as re-sourcing an import, belongs in the plan's Deviations section: `python scripts/implement-trajectory-log.py --event --run-id {run_id} --type deviation --step N --reason "<why>" --what-changed "<vs plan>"`. A notes-only `step_end status: ok` is NOT sufficient for a plan-text divergence.

**Wave execution mode:**

When the plan has `### Wave` headers, execute waves in order with parallel-agent dispatch. Bracket each wave: `wave_start` comes before the wave's first `step_start`, and `wave_end` comes after all member `step_end`s. Hold the sequential-ordering invariant inside single-step waves. For a wave the run never executes, emit one wave-scoped `deviation` keyed to the wave's first step, and no `wave_start`/`wave_end` pair. The full wave procedure — the parallel sub-agent brief contents, the `wave_end.successes` reconciliation rule, and the deferred-wave `deviation` form — is in **`references/implement-details.md`**. Read it when a plan actually contains waves; most runs are sequential and never load it.
...
**Self-check** (skip if `--no-trajectory`): `python scripts/implement-trajectory-log.py --verify --run-id {run_id}`. This is **advisory** — it NEVER hard-fails a completed run (CEO sovereignty holds, matching the soft contract gate). But if it exits non-zero, the reported defects MUST reach the Report's Deviations section **verbatim, as a named defect list**. Do not gloss them as an optional footnote. A non-zero self-check that is not surfaced verbatim is itself a discipline violation. Run this self-check BEFORE any `git commit` or `git pull`. The run-level files reconciliation compares the current engine working tree against `run_start.git_head`. It is only meaningful while the tree holds only this run's changes. A re-run after a merge or pull over-flags the pulled files, which is expected and advisory, not a regression. No temp files are written anymore, so there is no `_tmp/` cleanup step.
...
- Never skip documentation propagation. After you create or modify a skill, script, reference file, or rule, update CLAUDE.md and templates/GETTING-STARTED.md. Update every other affected documentation target before you declare done
...
- Never work around the emit-time sequencing guard (exit `5`) by reordering or dropping markers to silence it. The guard means a `step_end` or `step_start` is genuinely out of order. Emit the missing marker, or open a parallel `wave_start`, instead
