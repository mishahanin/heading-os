...
| `persistence.md` | Always-on | State lives in files, in any format that fits, and the workspace's database is SQLite. Postgres, Oracle, MySQL, Microsoft SQL Server, Redis, Qdrant, Neo4j and every other SERVER database is ruled out rather than weighed, because a store that outlives the process that wrote to it buys a daemon, a port and a backup story for a durability guarantee WSL2 does not document anyway. An in-process store adds none of that and is not ruled out. Rationale in `docs/ARCHITECTURE.md` § 4. |
