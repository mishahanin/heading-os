...
| `push-all.py` prints "could not verify the visibility of ..." and continues | The GitHub API could not answer (no token, no network, a rate limit, or a repository the token cannot see). The offline check still ran and passed. This is a warning, not a failure. |
