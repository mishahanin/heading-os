"""Tests for scripts/memlog.py. Run: python -m pytest tests/test_memlog.py -q

Ported from BMAD's bmad-brainstorming memlog test. The spine under test is the flat,
append-only, chronological invariant: every entry is one line recorded at the end in the
order it happened — no sections, no grouping.
"""
import json
import sys
...
def test_set_can_add_new_field(ws):
    init(ws)
    memlog.main(["set", "--workspace", ws, "--key", "owner", "--value", "Misha"])
    assert memlog.split(read(ws))[0]["owner"] == "Misha"
