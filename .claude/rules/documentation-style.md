...
The real debt was 217 errors, and the corpus was brought to **1** on 2026-08-17 — that one being `.claude/skills/ast-grep/SKILL.md`, vendored from upstream and pinned by content hash in `skills-lock.json`, so it is not ours to rewrite. `--skills` is still not a gate. Arming one needs an explicit vendored-skill exemption first, and that is Misha's call, not a checker's. Bring a skill down when you edit it, run `--skills --quiet` to see where the number stands, and do not read a green `--all` as a green corpus.
