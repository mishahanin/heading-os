...
- **Engine ⟂ data separation.** Seven enforcement layers (a static bypass guard, a leak guard, a data-path redirect, a build partition, a runtime tree-clean check, an unbypassable push-time wall in pure code, and a content guard) ensure the engine clone holds no private or personal data, regardless of how a file was written, and that none of it leaves on the push. The first six answer *where a file goes*; the content guard is the only one that opens an engine-routed file and reads *what is inside it*, which is how it caught a category of private content the routing map had missed. Specified in [`docs/engine-data-segregation-contract.md`](docs/engine-data-segregation-contract.md).
