...
- **The YARD bootstrap built a checkout that could not run the suite.** Step 4 of `scripts/herdr/heading-os-yard/yard-bootstrap.sh` ran a bare `uv sync`, installing core dependencies only, while `CLAUDE.md` § Setup has prescribed `uv sync --all-extras --group dev` for a fresh clone all along. Measured 2026-09-03 in a YARD against HELM: `import telethon` and `import cryptography` both succeeded in HELM and both raised `ModuleNotFoundError` in the YARD, and the full suite reported **229 failures** that a HELM-shaped environment does not produce. This was not a slow-start trade. A YARD exists so engine work can be judged in isolation, and a task that cannot separate its own regression from 229 failures it did not cause has lost the thing the YARD was for; the preceding commit on this branch had to spend a baseline run in a throwaway worktree to establish that its failures were pre-existing. After the fix the same suite reports **99**, and the survivors are the checkout being a worktree without the private data overlay rather than absent packages. `--all-extras` is used over `--extra all` because the `all` extra is an aggregate of the other nine and the flag needs no maintenance when a tenth lands. Mutation 8/8.

- **A bootstrap test assumed the checkout it ran from was HELM.** `test_the_status_file_it_reads_is_the_one_inside_the_worktree` read `ROOT / ".claude" / ".yard-bootstrap-status"` and asserted it did not exist, under the message "HELM carries a YARD status file". `ROOT` is whichever checkout the suite was launched from, and every YARD carries that receipt by design, so the test failed in the only place engine work happens and passed only in a bare checkout. It now asks `scripts/utils/clone_guard.is_main_clone()`. Rewritten rather than skipped: a provisioned worktree's receipt is a genuine confounder, so the assertion proves MORE from a YARD than it ever did from HELM, and a skip would have discarded the stronger half. Mutation 5/5.

- **Bootstrap tests talked to the herdr server, which is one server for the machine.** Every test that executed `yard-bootstrap.sh` reached the operator's live herdr: `workspace get`, `workspace rename`, `pane report-metadata` on every step, `pane run`, and two `notification show`. Found 2026-09-03 when a mutation run over those tests put "YARD: the engine/data contour is broken — step 6" on the operator's screen, describing a worktree that was fine. The notification was the visible half. The silent half is that the script reads `HERDR_WORKSPACE_ID` and `HERDR_PANE_ID` from the environment, the helpers built theirs from `os.environ`, and inside a herdr session both name the operator's own workspace and pane — so `workspace rename` and `pane run` were aimed at a live pane belonging to whatever else was running. Same shape as the `git worktree prune` defect repaired in `tests/conftest.py` the day before, against a different shared resource.

  The seam existed and nothing used it: line 41 of the bootstrap is `HERDR="${HERDR_BIN_PATH:-herdr}"`. One `herdr_stub` fixture in `tests/conftest.py` now fills it, records every call, and drops the two live identifiers so there is no target left to name; all six execution sites go through it rather than carrying copies. The second direction is asserted mechanically rather than by intent: the corpus is re-run with a poisoned `herdr` first on PATH, and both the suite staying green AND the poison log staying empty are required — almost every herdr call in the script ends in `|| true`, so a non-zero exit alone would be swallowed and green would prove nothing. An AST check additionally requires every bootstrap-executing test to name the fixture, since a future one that forgets is how this returns. Mutation 5/5.

  Two follow-on repairs came out of that work. `_run_against` now puts a stub `uv` on PATH, because step 4's new flags turned two tests in that module into 230-second runs that timed out under `-n auto`; the flags themselves are asserted textually instead, and the module docstring stopped claiming those two tests did not exist. And both of them now assert the run reached a step past 4 — they check for the ABSENCE of a phrase, which a bootstrap that died at step 4 satisfies just as well, and three mutations against the stub survived until that assertion existed. Mutation 3/3 on the stub.

- **A YARD's runtime receipt was committable to a public repository.** `.claude/.yard-bootstrap-status` records the bootstrap status, the step reached and a timestamp for ONE checkout, and no `.gitignore` rule matched it. Measured 2026-09-03 on a YARD: `git check-ignore -v .claude/.yard-bootstrap-status` exited 1 and the file sat untracked in `git status`, so it would have shown in every YARD forever and a `git add -A` inside any task would have published it. The engine-tree-clean wall does not cover this — it hunts artifacts routing `private`, and `.claude/` routes `engine`, so a runtime marker underneath it is the one shape that wall was never built to see. The new rule is deliberately narrow: `.claude/` holds 471 tracked engine files, and a wider glob would have hidden skills and rules from git rather than one receipt. Mutation 5/5, three of the five being over-wide rules rather than absent ones.

- **The version gate read three markdown files and not the lockfile.** `scripts/check-version-sync.py` asserted README, CHANGELOG and ROADMAP agreed with `pyproject.toml` and never opened `uv.lock`. Measured 2026-09-03 on `main`: the lock held `version = "0.13.0"` for `heading-os-engine` while pyproject declared `0.14.0`, the drift had survived the entire v0.14.0 release, and the guard printed "Version in sync: 0.14.0" and exited 0. The operational cost was per-worktree: step 4 of the YARD bootstrap runs `uv sync`, which rewrites the stale version on the spot, so every new YARD opened with `M uv.lock` already dirty and its operator had to work out whether that edit was theirs. The package is located by matching `project.name`, never by a hardcoded string, so a rename cannot silently reduce the check to nothing. `uv.lock` also joined the `files:` pattern of the `version-sync` pre-commit hook — without that a commit touching only the lock never fires the gate, which is how the drift survived. Mutation 6/6, one of them being the hook scope rather than the script.
