...
- For non-trivial code, the metric seeds the `/pre-impl` test-contract; run the full chain `/create-plan` then `/pre-impl` then `/implement` there. Do not duplicate `/pre-impl`'s heavy critique here.
