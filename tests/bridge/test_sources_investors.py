...
def test_parses_regional_row(tmp_path):
    _write_shortlist(tmp_path,
        ...
        "|---|------|------|----|--------|-----|-------|\n"
        "| 8 | Acme Capital | Independent growth VC | Hamburg | EUR 20-60M | HIGH | operator DNA via parent group |\n"
    )
    result = list_investors(tmp_path)
    assert result["total"] == 1
    assert result["raise_target"] == "$10-20M"
    f = result["firms"][0]
    assert f["num"] == 8
    assert f["firm"] == "Acme Capital"
...
def test_decisions_locked_assigns_status(tmp_path):
    _write_shortlist(tmp_path,
        ...
        "| # | Firm | Type | HQ | Cheque | Fit | Notes |\n"
        "|---|------|------|----|--------|-----|-------|\n"
        "| 8 | Acme Capital | VC | Hamburg | EUR 20M | HIGH | Notes |\n\n"
        "# Decisions locked\n\n"
        "## In-scope firms\n\n"
        "| Slot | Firm | Wave | Notes |\n"
        "|------|------|------|-------|\n"
        "| First 5 (this week) | Acme Capital (Vela) | Week 1 | Send-ready |\n"
    )
    result = list_investors(tmp_path)
...
def test_acronym_match_for_nif(tmp_path):
    """NIF in decisions table matches 'Northwind Innovation Fund' in regional table."""
    _write_shortlist(tmp_path,
        "## Europe (1)\n\n"
        "| # | Firm | Type | HQ | Cheque | Fit | Notes |\n"
        "|---|------|------|----|--------|-----|-------|\n"
        "| 11 | Northwind Innovation Fund (NIF) | VC | Amsterdam | EUR 5-15M | HIGH | mandate |\n\n"
        "# Decisions locked\n\n"
        "## In-scope firms\n\n"
        "| Slot | Firm | Wave | Notes |\n"
        "|------|------|------|-------|\n"
        "| Parallel-track Week 1-2 | NIF (Alvarez) | Week 1-2 | Send-ready |\n"
    )
    result = list_investors(tmp_path)
...
def test_first_token_match_for_contoso(tmp_path):
    ...
    _write_shortlist(tmp_path,
        ...
        "| Slot | Firm | Wave | Notes |\n"
        "|------|------|------|-------|\n"
        "| First 5 (this week) | Contoso CCI (Marchetti) | Week 1 | Send-ready |\n"
    )
    result = list_investors(tmp_path)
    assert result["firms"][0]["status"] == "first-5"
...
def test_dossier_path_matched_by_number(tmp_path):
    ...
    _write_shortlist(tmp_path,
        ...
        "| # | Firm | Type | HQ | Cheque | Fit | Notes |\n"
        "|---|------|------|----|--------|-----|-------|\n"
        "| 8 | Acme Capital | VC | Hamburg | 20M | HIGH | x |\n"
    )
    _write_dossier(tmp_path, "08-acme.md")
    _write_message(tmp_path, "08-acme-first-touch.md")
    result = list_investors(tmp_path)
    f = result["firms"][0]
    assert f["dossier_path"].endswith("/dossiers/08-acme.md")
    assert f["message_path"].endswith("/messages/08-acme-first-touch.md")


def test_out_of_scope_initech_captured(tmp_path):
    """Free-text 'Initech+' mention assigns out-of-scope status to 'Initech Capital'."""
    _write_shortlist(tmp_path,
        "## UK / Israel (1)\n\n"
        "| # | Firm | Type | HQ | Cheque | Fit | Notes |\n"
        "|---|------|------|----|--------|-----|-------|\n"
        "| 19 | Initech Capital (Initech+ growth platform) | Cyber VC | Tel Aviv | 10-30M | MED-HIGH | x |\n\n"
        "# Decisions locked\n\n"
        "## Out-of-scope this round\n\n"
        "- **Initech+** -- dropped this round to avoid overlap with an existing lead.\n"
    )
    result = list_investors(tmp_path)
...
def test_read_dossier_happy_path(tmp_path):
    _write_dossier(tmp_path, "08-acme.md", content="# Acme Capital\n\nDeep notes here.")
    rel_path = f"{PROGRAM_DIR}/dossiers/08-acme.md"
    r = read_dossier(tmp_path, rel_path)
    assert r["ok"] is True
    assert "Acme Capital" in r["content"]
    assert r["size"] > 0
...
def test_mark_sent_writes_log_entry(tmp_path):
    r = mark_sent(tmp_path, 7, note="sent via email to Delgado")
    assert r["ok"] is True
    assert r["firm_num"] == 7
...
def test_list_investors_joins_send_log(tmp_path):
    ...
    _write_shortlist(tmp_path,
        ...
        "|---|------|------|----|--------|-----|-------|\n"
        "| 8 | Acme Capital | VC | Hamburg | 20M | HIGH | x |\n"
        "| 9 | Globex Ventures | VC | Zurich | 30M | HIGH | x |\n"
    )
    mark_sent(tmp_path, 8, note="Vela, LinkedIn")
    result = list_investors(tmp_path)
    by_num = {f["num"]: f for f in result["firms"]}
    assert by_num[8]["sent_date"]
    assert by_num[8]["sent_note"] == "Vela, LinkedIn"
