...
|---|---|---|---|
| Four backward-compat hook shims -- `.claude/hooks/{prevent-secrets,protect-corporate,protect-docs,protect-personal-threads}.py` (28-line delegators to `_dispatch.py`) | Every exec's `settings.local.json` referencing `_dispatch.py` directly instead of the per-hook script names | All provisioned execs have re-synced a `settings.local.json` that points PreToolUse at `_dispatch.py` (verify via the next fleet `git pull` round, then `git rm` the four shims AND the `prevent-secrets.py` entry in `scripts/secret-scanner.py`'s exclusion list, in one change) | Open, but no longer self-renewing. `provision-exec.py` wrote a fresh `protect-corporate.py` reference into every new workspace until 2026-08-09, so the removal condition could never be reached; it now emits `_dispatch.py` on all three matchers, held by `tests/test_provision_exec_settings.py`. Remaining blocker is genuinely just the existing fleet. (The fifth shim, `protect-secure.py`, went with the vault in Plan 5.) |
