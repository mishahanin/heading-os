...
## 7. Install dependencies & arm both gates

```bash
cd <engine>
uv sync                          # core deps only - the light always-on set
pre-commit install               # commit-time secret scanner (once per fresh clone)
bash scripts/setup-platform.sh   # session hooks (once per fresh clone)
```
...
> 16 of the 17 session hooks, `_dispatch.py` among them. `_dispatch.py` is the
> single entry point for eleven PreToolUse walls, including the release gate.
> A clone that skips this step arms one hook, from the tracked
> `.claude/settings.json`, and nothing else reports the other 16 as absent.
> Verify with `bash scripts/setup-platform.sh --check`, which exits non-zero and
> names every missing registration. It compares two files, so it establishes
> that a hook is REGISTERED, never that a hook ran. Running the step again on an
> existing clone is safe: it merges rather than overwrites, keeping every local
> key. `/prime` runs the same check at every session boot and stays silent when
> the clone is armed.
...
| The commit secret gate never fires (a bad commit goes through) | The `pre-commit install` step in §7 was skipped. Run `uv run pre-commit install` in the engine clone; verify with `uv run python scripts/install-hooks.py --check`, which exits non-zero unless the hook file git would run exists, was generated by the pre-commit framework, and is executable. It does not run the hook, so a zero exit says the gate is armed, not that its checks pass. |
| No hook ever fires: no session-start brief, no PreToolUse wall, no release gate | The `bash scripts/setup-platform.sh` step in §7 was skipped, so `.claude/settings.local.json` does not exist. It is gitignored and registers 16 of the 17 session hooks. Run the step, then verify with `bash scripts/setup-platform.sh --check`, which names every missing registration and exits non-zero. It compares two files, so a zero exit says a hook is registered, not that a hook ran. |
