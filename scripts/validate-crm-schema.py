...
def parse_frontmatter(path: Path) -> dict | None:
    ...
    The READING half, kept byte-identical for every caller that holds a path.
    The parser is :func:`parse_frontmatter_text` below, which carries the reason
    this file keeps its own; the split is not cosmetic. A caller that ALREADY
    has the text -- because it walked the tree and read each record through
    ``scripts.utils.repo_files.read_sources`` -- calls that one directly and
    never re-opens the path. Re-opening is a second window in which a record
    written and removed by a parallel agent raises FileNotFoundError, and this
    function would turn that into a FAIL against a record nobody could fault.
    """
    try:
...
def parse_frontmatter_text(text: str) -> dict | None:
    """Parse already-read record text. Returns None when frontmatter is missing.
