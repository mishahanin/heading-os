...
| `add` | Warn when `email` contains `@31c.io` and the type is neither `tribe` nor `tribe-leadership`. On the CEO workspace only, and only for a deal-bearing external type: `python scripts/odin-principles.py --type {relationship_type} [--stage {stage}] --json`. | Create the address book entity plus the relationship record, then index the contact in `context/people.md`. |
