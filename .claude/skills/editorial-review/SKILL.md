...
5. **Optional — pull a critique method.** For a hard restructure, run `python scripts/elicit.py show "Critique and Refine"` or `"Red Team vs Blue Team"` from the shared elicitation catalog. Apply it to the argument arc. Skip it when the checklist already bites.
