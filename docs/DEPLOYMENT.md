...
| No hook ever fires: no session-start brief, no PreToolUse wall, no release gate | The `bash scripts/setup-platform.sh` step in §7 was skipped, so `.claude/settings.local.json` does not exist. It is gitignored and registers 15 of the 17 session hooks. Run the step, then verify with `bash scripts/setup-platform.sh --check`, which names every missing registration and exits non-zero. It compares two files, so a zero exit says a hook is registered, not that a hook ran. |
